...
async Task GetResponseFromOpenAI(string prompt)
{
    ...
    string GetHashCode = completion.GetHashCode().ToString();
    // Write the file.
    System.IO.File.WriteAllText($"result/{GetHashCode}.txt", completion);

    // Write response to console
    Console.WriteLine($"\nResponse written to result/{GetHashCode}.txt\n\n");
}